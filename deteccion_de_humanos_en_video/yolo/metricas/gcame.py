...
class GCAME(object):
    ...
    def _sigma_gcame(self, H, W, h, w, scale_factor=1.0):
        ...
        sigma = max(abs((math.log(S + 1e-6) / kn_size)) * scale_factor, 1.0)
        return float(sigma)
    ...
    def forward_ultralytics_yolo(self, img_np, box_xyxy, cls_id=None, device=None):
        ...
        sigma = self._sigma_gcame(H, W, h, w, scale_factor = 2.0) #sigma haundigoa iteko, para que no se centralice tanto
