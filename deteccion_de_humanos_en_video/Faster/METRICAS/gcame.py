...
class GCAME(object):
    ...
    def forward_fasterrcnn(self, img, box, index=None):
        ...
        for layer in self.target_layers:
            ...
            # 1. CAM está en resolución layer4 (Hc, Wc)
            cam = np.maximum(cam, 0)

            # 2. Reescalar CAM al tamaño de la imagen
            cam_full = cv2.resize(cam, (org_size[1], org_size[0]))

            # 3. Recortar SOLO la ROI
            roi_cam = cam_full[y1:y2+1, x1:x2+1]

            # 4. Normalizar SOLO dentro de la ROI (si es cero que no calcule el mínimo porque sino da error)
            if roi_cam.size == 0:
                return None
            ...
            roi_cam /= (roi_cam.max() + eps)

            # 5. Insertar en mapa final
            score_saliency_map[y1:y2+1, x1:x2+1] = roi_cam
